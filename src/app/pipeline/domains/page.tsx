...
export default function DomainPipelinePage() {
    ...
    const handleSeed = async () => {
        if (!subjectId.trim()) {
            setStatus({ type: 'error', message: "Subject ID is required" });
            return;
        }
        ...
        try {
            ...
            const validation = DomainIngestionSchema.safeParse(parsed);
            ...
            const response = await fetch("/api/pipeline/domains", {
                method: "POST",
            ...
            if (result.success) {
                setStatus({
                    type: 'success',
                    message: `Successfully seeded ${result.data.count} domains!`
                });
            } else {
                setStatus({
                    type: 'error',
        ...
        <main style={{
            ...
            <header>
                ...
                <p style={{ color: 'var(--text-secondary)' }}>
                    Bulk seed domains under a specific subject.
                </p>
            </header>
            ...
            <section style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <textarea
                        ...
                        placeholder={`[\n  {\n    "slug": "algebra",\n    "title": "Algebra",\n    "order": 1,\n    "summary": "The study of mathematical symbols"\n  }\n]`}
                        style={{
                            width: '100%',
                            height: '300px',
                            background: '#0a0e14',
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-4)' }}>
                    ...
                    >
                        {status.type === 'loading' ? 'Processing...' : 'Seed Domains'}
                    </button>
                </div>
            </section>
